...
#Region "Privates"
    ...
    Private _standard61 As Boolean = False
    Private _earthFriendly As Boolean = False
...
#End Region
    ...
    Protected Sub GenPDF(ByVal ProductID As Long)
        ...
        Dim books = XDocument.Load(webFolder & System.Configuration.ConfigurationManager.AppSettings("XML_ChemicalsFile"))
        ...
        If LetterRecord.Count = 1 Then
            ...
            _earthFriendly = CBool(Trim(Letter.SelectSingleNode("earthFriendly").InnerText))
            _standard61 = CBool(Trim(Letter.SelectSingleNode("standard61").InnerText))
            ...
            Dim txtPageType As String = tdsLangInfo.chemPageType
            ...
            Dim DestPDF = Server.MapPath("/pdfs/tds_chemical_" & bGuid.ToString & ".pdf")
            ...
            Try
                ...
                Dim MyHeaderWritten As Boolean = False
                    ...
                    If ((_attr_value.ToString.Length > 1) Or IsNumeric(_attr_value)) Or (_attr_type = "12") Then
                        Select Case _attr_type
                            ...
                            Case "3"
                                ...
                                For Each crPara As String In _attr_value.Split(vbCrLf)
                                    crPara = crPara.Replace("<br />", " ")

                                    If crPara.Trim().Length = 0 Then
                                        OutputString.Append(vbCrLf & vbCrLf)
                                    Else
                                        OutputString.Append(crPara)
                                    End If
                                Next
                                ...
                                If LastHeaderWritten = False AndAlso LastHeaderID = _attr_header Then
                                    ...
                                    If _attr_showonsheet = True Then
                                        ...
                                        Dim paras As String() = OutputString.ToString.Split(vbCrLf)
                                        For Each para As String In paras
                                            para = para.Replace(vbCrLf, " ")
                                            If para.Length > 2 Then
                                                If MyHeaderWritten Then
                                                    WriteNonHeadedParagraph(doc, ct, column, para, smAttrFont, True, "", Nothing)
                                                Else
                                                    WriteHeadedParagraph(doc, ct, column, _attr_name, smAttrHdrFont, para, smAttrFont, True, LastHeader, AttrHdrFont)
                                                    '  WriteHeadedParagraph(doc, ct, column, _attr_name, smAttrHdrFont, para, smAttrFont, True)
                                                    MyHeaderWritten = True
                                                End If
                                                '                                            WriteParagraphHTML(doc, ct, column, para, AttrFont, AttrHdrFont)
                                            End If
                            ...
                            Case "6", "11", "12"
                                If _attr_showonsheet = True Then
                                    ...
                                    If _attr_type = "11" Then
                                        'WriteHeadedParagraph(doc, ct, column, PreText.Attribute, AttrHdrFont, "", AttrFont, False)
                                        Dim AttrHdrChunkPre As Chunk = New Chunk(_attr_name & vbCrLf, AttrHdrFont)
                                        Dim AttrHdrPhrasePre As Phrase = New Phrase(AttrHdrChunkPre)
                                        attrHdrPara.Add(AttrHdrPhrasePre)
                                    End If
                                    ...
                                    ' Adding Standard 61 image (if applicable)
                                    If _attr_name.ToLower = "features" And _standard61 Then
                                        Dim imgPara As Paragraph = New Paragraph

                                        Dim standard61image As String = Server.MapPath("/images/standard61.png")
                                        Dim std61width As Single = 95


                                        Dim img61 As iTextSharp.text.Image = iTextSharp.text.Image.GetInstance(standard61image)
                                        img61.ScaleAbsoluteWidth(std61width)

                                        CheckStandard61ImageColumns(status, ct, column, doc, YLine)
                                        AddImage(ct, imgPara, img61)
                            ...
                            Case "4"
                                ...
                                Select Case _attr_grid
                                    Case "Grid_TestResults"
                                        GridID = 1
                                        GridXMLName = "Grid_TestResults"
                                    Case "Grid_Properties"
                                        GridID = 7
                                        GridXMLName = "Grid_Properties"
                                    Case "Grid_TensionLoads"
                                        GridID = 5
                                        GridXMLName = "Grid_TensionLoads"
                                    Case "Grid_Usage"
                                        GridID = 6
                                        GridXMLName = "Grid_Usage"
                                    Case "Grid_Estimating"
                                        GridID = 4
                                        GridXMLName = "Grid_Estimating"
                                    Case "Grid_Packaging"
                                        GridID = 2
                                        GridXMLName = "Grid_Packaging"
                                    Case "Grid_CureTime"
                                        GridID = 3
                                        GridXMLName = "Grid_CureTime"
                                    Case "Grid_Accelerator"
                                        GridID = 8
                                        GridXMLName = "Grid_Accelerator"
                                    Case "Grid_Accessory"
                                        GridID = 9
                                        GridXMLName = "Grid_Accessory"
                                    Case "Grid_Primer"
                                        GridID = 10
                                        GridXMLName = "Grid_Primer"
                                    Case "Grid_Catalyst"
                                        GridID = 11
                                        GridXMLName = "Grid_Catalyst"
                                    Case "Grid_Aggregate"
                                        GridID = 12
                                        GridXMLName = "Grid_Aggregate"
                                    Case "Grid_Aggregate2"
                                        GridID = 13
                                        GridXMLName = "Grid_Aggregate2"
                                    Case Else
                                        GridID = 0
                                End Select
                        ...
                        End Select
                        '                    Select Case _attr_headerType.ToString
                ...
                ' Manufacturer
                WriteHeadedParagraph(doc, ct, column, tdsLangInfo.chemManufacturerHeader, AttrHdrFont, tdsLangInfo.chemManufacturer, AttrFont, True)

                ' warranty area
                Dim warrHdrPara As Paragraph = New Paragraph
                warrHdrPara.SetLeading(leadingHdrF, leadingHdrM)
                Dim warrHdrChunk As Chunk = New Chunk(tdsLangInfo.chemWarrantyHeader, AttrHdrFont)
                ...
                Dim warrChunk As Chunk = New Chunk(tdsLangInfo.chemWarranty1, NoteFont)
    ...
    Protected Sub WriteNonHeadedParagraph(ByRef doc As Document, ct As ColumnText, ByRef column As Integer, ValueText As String, AttrFont As Font, SkipHeaderSpacing As Boolean, Optional ByRef SectionHdr As String = "", Optional SectionHdrFont As Font = Nothing)

        Dim Status As Integer
        Dim yLine As Double

        ' this is a bit less space
        'ValueText = ValueText.Trim()

        ' this is a bit more space between
        If ValueText.StartsWith(vbCrLf) Or ValueText.StartsWith(vbCr) Or ValueText.StartsWith(vbLf) Then
            ValueText = ValueText.Substring(1, ValueText.Length - 1)
        End If

        'If ValueText.EndsWith(vbCrLf) Or ValueText.EndsWith(vbCr) Or ValueText.EndsWith(vbLf) Then
        '    ValueText = ValueText.Substring(0, ValueText.Length - 1)
        'End If
        ' ValueText = ValueText.Replace(vbCrLf, "").Replace(vbCr, "").Replace(vbLf, "")
    ...
    Protected Sub CheckStandard61ImageColumns(ByRef Status As Integer, ByRef ct As ColumnText, ByRef column As Integer, ByRef doc As Document, ByRef yLine As Double)
        Status = ct.Go(True)
